...
def prepare_list_words(path, rows=ROWS_COUNT):
    if rows:
        ...
        print_sorted_list(wc.items(), rows=ROWS_COUNT, columns=0, print_solution=True)
...
def prepare_list(path, sort_by_freq=False):
    ...
    print_sorted_list(collection, rows=ROWS_COUNT, columns=cols_cnt, print_solution=False)
